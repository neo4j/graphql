...
export class ComplexityEstimatorHelper {
    ...
    public registerField(parentObjectTypeNameOrInterfaceTypeName: string, fieldName: string): void {
        if (this.useComplexityEstimators) {
            const existingFieldsForTypeName =
                this.objectTypeNameToFieldNamesMapForComplexityExtensions.get(
                    parentObjectTypeNameOrInterfaceTypeName
                ) ?? [];
            this.objectTypeNameToFieldNamesMapForComplexityExtensions.set(
                parentObjectTypeNameOrInterfaceTypeName,
                existingFieldsForTypeName.concat(fieldName)
            );
        }
    }
    ...
    public hydrateDefinitionNodeWithComplexityExtensions(definition: DefinitionNode): DefinitionNode {
        ...
        const fieldsWithComplexity = definition.fields?.map((f) => {
            const hasFieldComplexityEstimator = this.getFieldsForParentTypeName(definition.name.value).find(
                (fieldName) => fieldName === f.name.value
            );
