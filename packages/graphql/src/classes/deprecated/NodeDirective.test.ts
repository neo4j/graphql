...
describe("NodeDirective", () => {
    ...
    test("should prefer labels over additional labels + label", () => {
        const instance = new NodeDirective({ labels: ["Something"], additionalLabels: ["Label1", "Label2"] });
        const labelString = instance.getLabelsString("MyLabel", defaultContext);

        expect(labelString).toBe(":`Something`");
    });
});
