...
// function validateFulltextDirective(context: Neo4jValidationContext): ASTVisitor {
...
export function validateLimitDirective(context: Neo4jValidationContext): ASTVisitor {
    ...
    return {
        InterfaceTypeDefinition(
            ...
            const { isValid, errorMsg, errorPath } = assertValid(() => {
                // TODO: we should check that all the implementing types are of type node
                if (defaultLimit) {
                ...
                if (maxLimit) {
                    ...
                    if (maxValue <= 0) {
                        throw new DocumentValidationError(
                            `@limit.max invalid value: ${maxValue}. Must be greater than 0.`,
                            ["max"]
                        );
                    }
                }
