...
describe("validateDocument", () => {
    ...
    describe("Authorization", () => {
        test("@authorization on object should throw without features", () => {
            const doc = gql`
        ...
        test("@authorization on field should throw without features", () => {
            const doc = gql`
                type User {
                    id: ID!
        ...
        test("@authorization on object should not throw with feature", () => {
            const doc = gql`
                type User @authorization(validate: [{ where: { node: { id: "$jwt.sub" } } }]) {
                    id: ID!
                    password: String!
            ...
            expect(() =>
                validateDocument({ document: doc, features: { authorization: { key: "secret" } } })
            ).not.toThrow();
        });

        test("@authorization on field should not throw with feature", () => {
            const doc = gql`
                type User {
                    id: ID!
                    password: String! @authorization(validate: [{ where: { node: { id: "$jwt.sub" } } }])
