...
function getObjFieldMeta({
    ...
    return obj?.fields?.reduce(
        (res: ObjectFields, field) => {
            ...
            const autoGenerated = field?.directives?.find((x) => x.name.value === "autogenerated");
            ...
            const baseField: BaseField = {
                ...
                otherDirectives: (field.directives || []).filter(
                    (x) => !["relationship", "cypher", "autogenerated"].includes(x.name.value)
                ),
    ...
    neoSchemaInput.nodes.forEach((node) => {
        ...
        const nodeInput = composer.createInputTC({
            ...
            fields: [...node.primitiveFields, ...node.scalarFields, ...node.enumFields].reduce((res, f) => {
                if ((f as PrimitiveField)?.autogenerated) {
