...
function mergeStatement({
    ...
    const relationshipMerge = new CypherBuilder.Query()
        .merge(relationship)
        .onCreate({ relationship: convertToCypherParams(input.onCreate?.edge || {}) });
...
function getAutogeneratedParams(node: Node): Record<string, Param<any>> {
    ...
    //     .filter((field) => field.autogenerate && !Object.keys(sourceNode.parameters ?? []).includes(field.fieldName))
    //     .forEach((field) => {
    //         onCreateStatements.push([`${varName}.${field.dbPropertyName} = randomUUID(),\n`, {}]);
    //     });
    // node.temporalFields
    //     .filter((field) => ["DateTime", "Time"].includes(field.typeMeta.name) && field.timestamps?.includes("CREATE"))
    //     .forEach((field) => {
    //         onCreateStatements.push([
    //             `${varName}.${field.dbPropertyName} = ${field.typeMeta.name.toLowerCase()}(),\n`,
    //             {},
    //         ]);
    //     });
}
