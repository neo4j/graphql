...
export class UpdateFactory {
    ...
    private hydrateUpdateOperation({
        ...
        asArray(input).forEach((inputItem) => {
            ...
            // const resolveTreeWhere: Record<string, any> = args.where ?? {};
            //     this.queryASTFactory.operationsFactory.getWhereArgs(resolveTree);
            // console.log("ResolveTree", resolveTree);
                ...
                if (attribute) {
                    if (operator) {
                        ...
                        for (const op of Object.keys(targetInput[fieldName])) {
                            const paramInputField = this.getInputField(op, attribute, targetInput[fieldName][op]);
                            update.addField(paramInputField);
