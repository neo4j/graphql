...
export class CreateFactory {
    ...
    private hydrateUnwindCreateOperation({
        ...
        const target = isNested ? entityOrRel.target : entityOrRel;
        assertIsConcreteEntity(target);
        ...
        asArray(input).forEach((inputItem) => {
            if (isNested) {
                this.raiseAttributeAmbiguity(Object.keys(inputItem.node ?? {}), target);
                this.raiseAttributeAmbiguity(Object.keys(inputItem.edge ?? {}), entityOrRel);
            } else {
                this.raiseAttributeAmbiguity(Object.keys(inputItem ?? {}), target);
            }
            const targetInput = isNested ? inputItem["node"] ?? {} : inputItem ?? {};
            for (const key of Object.keys(targetInput)) {
                const relationship = target.relationships.get(key);
    ...
    private addAutogeneratedFields({
        ...
        entity.attributes.forEach((attribute) => {
            ...
            if (attribute.timestampCreateIsGenerated()) {
                ...
                unwindCreate.addField(inputField);
            }
