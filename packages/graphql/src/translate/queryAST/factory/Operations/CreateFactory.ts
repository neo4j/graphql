...
export class CreateFactory {
    ...
    private hydrateUnwindCreateOperation({
        ...
        asArray(input).forEach((inputItem) => {
            ...
            raiseAttributeAmbiguity(Object.keys(inputItem.edge ?? {}), relationship);
...
function assertIsDefined<T>(t?: T): asserts t is T {
    if (t === undefined) {
        throw new Error("Transpile Error: Expected value to be defined");
    }
}
