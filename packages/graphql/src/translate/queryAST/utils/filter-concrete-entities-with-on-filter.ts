 ...
 **/
export function filterConcreteEntities(
    compositeTarget: UnionEntityAdapter | InterfaceEntityAdapter,
    whereArgs?: Record<string, any>
): ConcreteEntityAdapter[] {
    if (isUnionEntity(compositeTarget)) {
        return filterConcreteEntitiesWithOnFilterUnion(compositeTarget, whereArgs);
    } else {
        return filterConcreteEntitiesWithOnFilterInterface(compositeTarget, whereArgs);
    }
}

function filterConcreteEntitiesWithOnFilterInterface(
...
function filterConcreteEntitiesWithOnFilterUnion(
