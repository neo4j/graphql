...
function createProjectionAndParams({
    ...
    function reducer(res: Res, [key, field]: [string, ResolveTree]): Res {
        ...
        if (cypherField) {
            ...
            const referenceNode = context.neo4jgraphql.nodes.find((x) => x.name === cypherField.typeMeta.name);
        ...
        if (relationField) {
            const referenceNode = context.neo4jgraphql.nodes.find((x) => x.name === relationField.typeMeta.name) as Node;
