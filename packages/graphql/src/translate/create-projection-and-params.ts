...
export default function createProjectionAndParams({
    ...
    function reducer(res: Res, field: ResolveTree): Res {
            ...
            if (relationField.interface || relationField.union) {
                ...
                for (const refNode of referenceNodes) {
                    ...
                    const nestedProjection = new Cypher.RawCypher((env) => {
                        ...
                        return `{ __resolveType: "${refNode.name}", __id: id(${(varName as any).getCypher(env)})${
                ...
                res.projection.push(
                    new Cypher.RawCypher((env) => `${alias}: ${(subqueryReturnAlias as any).getCypher(env)}`)
                );
        ...
        if (aggregationFieldProjection) {
            ...
            res.projection.push(
                new Cypher.RawCypher(
                    (env) => `${alias}: ${(aggregationFieldProjection.projectionCypher as any).getCypher(env)}`
                )
            );
            return res;
        }
        ...
        if (connectionField) {
            ...
            res.projection.push(
                new Cypher.RawCypher((env) => `${field.alias}: ${(returnVariable as any).getCypher(env)}`)
            );
        ...
        if (pointField) {
            ...
            res.projection.push(new Cypher.RawCypher((env) => `${field.alias}: ${(pointExpr as any).getCypher(env)}`));
            ...
            res.projection.push(
                new Cypher.RawCypher((env) => `${field.alias}: ${(datetimeExpr as any).getCypher(env)}`)
            );
        } else {
            ...
            if (alias !== field.name || dbFieldName !== field.name || literalElements) {
                res.projection.push(
                    new Cypher.RawCypher((env) => `${alias}: ${(varName.property(dbFieldName) as any).getCypher(env)}`)
                );
            } else {
    ...
    const { projection, params, meta, subqueries, subqueriesBeforeSort } = Object.values(mergedFields).reduce(reducer, {
        projection: resolveType
            ? [
                  ...
                  new Cypher.RawCypher((env) => `__id: id(${(varName as any).getCypher(env)})`),
              ]
            : [],
        params: {},
        meta: {},
    ...
    const projectionCypher = new Cypher.RawCypher((env) => {
        return `{ ${projection.map((proj) => (proj as any).getCypher(env)).join(", ")} }`;
    });
    return {
