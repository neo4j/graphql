...
function createCreateAndParams({
    ...
    timestamps.forEach((ts) => {
        const dbFieldName = mapToDbProperty(node, ts.fieldName);
        initial.push(`SET ${varName}.${dbFieldName} = datetime()`);
    });

    const autogeneratedIdFields = node.primitiveFields.filter((x) => x.autogenerate);
    autogeneratedIdFields.forEach((f) => {
        const dbFieldName = mapToDbProperty(node, f.fieldName);
        initial.push(`SET ${varName}.${dbFieldName} = randomUUID()`);
    });
