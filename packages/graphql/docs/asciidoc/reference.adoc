...
==== Authentication
This implementation will just expect there to be an authorization header in the request object, you can authenticate users however you like. One you could; Have a custom sign-in mutation, integrate with Auth0, or roll your own SSO server. The point here is that it's just a JWT, in the library, we will decode it to make sure it's valid... but it's down to you to issue tokens.
...
3. update - `SET`ing
4. delete - `DELETE`ing
5. connect - `MATCH`ing & `MERGE`ing
6. disconnect - `MATCH`ing & `DELETE`ing
...
Now we have two users in our database, and given the above GraphQL type definitions - How can we restrict `user1` from seeing `user2`? Great question, this is where allow comes in;
...
The generated cypher for this query would look along the lines of; 
...
Throwing us out of the operation because the ids do not match. This is just showcasing the `update` operation but bind is useful in the context of; `update`, `create`, `connect`, `disconnect`. 
