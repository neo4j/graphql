...
describe("@vector schema", () => {
    ...
    test("vector index with maxPhraseLength", async () => {
        const typeDefs = gql`
            type Movie
                @node
                @vector(
                    indexes: [
                        {
                            indexName: "MovieTitle"
                            embeddingProperty: "title"
        ...
        expect(printedSchema).toMatchInlineSnapshot(`
            ...
            type Query {
              movies(limit: Int, offset: Int, sort: [MovieSort!], where: MovieWhere): [Movie!]!
              moviesConnection(after: String, first: Int, sort: [MovieSort!], where: MovieWhere): MoviesConnection!
              titleQuery(after: String, first: Int, phrase: String!, sort: [MovieIndexSort!], where: MovieIndexWhere): MoviesIndexConnection!
            }
