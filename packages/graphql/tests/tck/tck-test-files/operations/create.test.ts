...
describe("Cypher Create", () => {
    ...
    test("Simple create -> relationship field -> connection(where)", async () => {
        ...
        expect(formatCypher(result.cypher)).toMatchInlineSnapshot(`
            ...
            } RETURN actorsConnection\\", { this0_movies: this0_movies, projection_movies_actorsConnection_args_where_Actorparam0: $projection_movies_actorsConnection_args_where_Actorparam0, projection_movies_actorsConnection: $projection_movies_actorsConnection, auth: $auth }, false) } ] }] AS data"
        `);
