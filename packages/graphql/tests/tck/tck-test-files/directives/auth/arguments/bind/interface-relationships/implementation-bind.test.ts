...
describe("Cypher Auth Allow", () => {
    ...
    test("Update Nested Node", async () => {
        ...
        expect(formatCypher(result.cypher)).toMatchInlineSnapshot(`
            ...
            YIELD value AS _
            WITH this, this_content0
            CALL apoc.util.validate(NOT ((exists((:\`User\`)-[:HAS_CONTENT]->(this_content0)) AND all(creator IN [(creator:\`User\`)-[:HAS_CONTENT]->(this_content0) | creator] WHERE (creator.id IS NOT NULL AND creator.id = $this_content0_auth_bind0_creator_id)))), \\\\\\"@neo4j/graphql/FORBIDDEN\\\\\\", [0])
            WITH this, this_content0
            CALL {
            	WITH this_content0
            	MATCH (this_content0)<-[this_content0_creator_User_unique:HAS_CONTENT]-(:User)
