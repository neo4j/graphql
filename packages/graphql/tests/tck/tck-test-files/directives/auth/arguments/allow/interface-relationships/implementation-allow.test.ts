...
describe("@auth allow on specific interface implementation", () => {
    ...
    test("Disconnect Node", async () => {
        ...
        expect(formatCypher(result.cypher)).toMatchInlineSnapshot(`
            ...
            CALL apoc.util.validate(NOT ((exists((:\`User\`)-[:HAS_CONTENT]->(this_disconnect_content0)) AND any(creator IN [(creator:\`User\`)-[:HAS_CONTENT]->(this_disconnect_content0) | creator] WHERE (creator.id IS NOT NULL AND creator.id = $this_disconnect_content0Post1_allow_auth_allow0_creator_id)))), \\"@neo4j/graphql/FORBIDDEN\\", [0])
            FOREACH(_ IN CASE WHEN this_disconnect_content0 IS NULL THEN [] ELSE [1] END |
            DELETE this_disconnect_content0_rel
            )
            RETURN count(*) AS _
    ...
    test("Nested Disconnect Node", async () => {
        ...
        expect(formatCypher(result.cypher)).toMatchInlineSnapshot(`
            ...
            CALL apoc.util.validate(NOT ((exists((:\`User\`)-[:HAS_CONTENT]->(this_disconnect_content0)) AND any(creator IN [(creator:\`User\`)-[:HAS_CONTENT]->(this_disconnect_content0) | creator] WHERE (creator.id IS NOT NULL AND creator.id = $this_disconnect_content0Post0_allow_auth_allow0_creator_id)))), \\"@neo4j/graphql/FORBIDDEN\\", [0])
    ...
    test("Connect node", async () => {
        ...
        expect(formatCypher(result.cypher)).toMatchInlineSnapshot(`
            ...
            UNION
            	...
            	CALL apoc.util.validate(NOT ((exists((:\`User\`)-[:HAS_CONTENT]->(this_connect_content0_node)) AND any(creator IN [(creator:\`User\`)-[:HAS_CONTENT]->(this_connect_content0_node) | creator] WHERE (creator.id IS NOT NULL AND creator.id = $this_connect_content0_nodePost0_allow_auth_allow0_creator_id)))), \\"@neo4j/graphql/FORBIDDEN\\", [0])
            	FOREACH(_ IN CASE WHEN this IS NULL THEN [] ELSE [1] END |
            		FOREACH(_ IN CASE WHEN this_connect_content0_node IS NULL THEN [] ELSE [1] END |
            			MERGE (this)-[:HAS_CONTENT]->(this_connect_content0_node)
            		)
