...
describe("Label in Node directive", () => {
    ...
    test("Update connection in Movie with label film", async () => {
        ...
        expect(formatCypher(result.cypher)).toMatchInlineSnapshot(`
            ...
            RETURN this, this_connect_actors0_node, [ metaVal IN [{type: 'Connected', name: 'Movie', relationshipName: 'ACTED_IN', toName: 'Actor', id: id(this), toID: id(this_connect_actors0_node)}] WHERE metaVal IS NOT NULL AND metaVal.id IS NOT NULL AND (metaVal.toID IS NOT NULL OR metaVal.toName IS NULL) ] as this_connect_actors0_node_mutateMeta
