...
describe("Field Level Aggregations", () => {
    ...
    test("Node Aggregations and Count", async () => {
        ...
        expect(formatCypher(result.cypher)).toMatchInlineSnapshot(`
            "MATCH (this:\`Movie\`)
            RETURN this { actorsAggregate: { count: size([(this_actorsAggregate_this0:\`Actor\`)-[this_actorsAggregate_this1:ACTED_IN]->(this) WHERE true | this_actorsAggregate_this0]), node: { name: head(apoc.cypher.runFirstColumn(\\"MATCH (this)<-[r:ACTED_IN]-(n:Actor)
                    WITH n as n
