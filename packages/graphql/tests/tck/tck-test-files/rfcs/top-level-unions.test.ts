...
describe("integration/rfs/top-level-unions", () => {
    ...
    test("should query simple top level unions with limit", async () => {
        const typeDefs = gql`
            type Movie {
                id: ID!
            }
        ...
        const query = gql`
            query {
                productions(options: { Movie: { limit: 1 }, Series: { limit: 1 } }) {
                    ... on Movie {
                        __typename
                        id
                    }
