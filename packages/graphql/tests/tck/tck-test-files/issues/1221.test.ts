...
describe("https://github.com/neo4j/graphql/issues/1221", () => {
    ...
    test("should apply where filter for deep relations, three relations deep", async () => {
        ...
        expect(formatCypher(result.cypher)).toMatchInlineSnapshot(`
            ...
            )\\\\\\", { this_mainConnection_SINGLE_Series_map_node_MasterData_map_node: this_mainConnection_SINGLE_Series_map_node_MasterData_map.node, this_mains: $this_mains })
            )\\", { this_mainConnection_SINGLE_Series_map_node: this_mainConnection_SINGLE_Series_map.node, this_mains: $this_mains }))
            CALL {
            WITH this
