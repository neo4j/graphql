...
describe("#288", () => {
    ...
    test("Can update a USER and COMPANYID is populated", async () => {
        ...
        expect(formatCypher(result.cypher)).toMatchInlineSnapshot(`
            ...
            RETURN [ metaVal IN [{type: 'Updated', name: 'USER', id: id(this), properties: $this_update}] WHERE metaVal IS NOT NULL AND metaVal.id IS NOT NULL ] as mutateMeta, this { .USERID, .COMPANYID } AS this"
        `);
