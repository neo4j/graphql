...
describe("Group By Directive - Top Level - @authorization validate on fields", () => {
    let typeDefs: string;
    ...
    test("Movies grouped by released and other, only project values - auth rule on released and other applies bc of values", async () => {
        const query = /* GraphQL */ `
            {
                moviesConnection {
        ...
        expect(formatCypher(result.cypher)).toMatchInlineSnapshot(`
            "CYPHER 5
            MATCH (this0:Movie)
            CALL apoc.util.validate(NOT ($isAuthenticated = true AND ($param1 IS NOT NULL AND this0.other = $param1)), '@neo4j/graphql/FORBIDDEN', [])
    ...
    test("Paginated movies + grouped movies, project nodes and grouped nodes - only auth rule on title applies", async () => {
        const query = /* GraphQL */ `
            {
    ...
    test("Paginated movies + grouped movies, project grouped nodes - auth rule on title applies", async () => {
        const query = /* GraphQL */ `
            {
                moviesConnection(first: 2) {
                    groupBy(fields: { released: true, other: true }) {
    ...
    test("Paginated aggregate on grouped movies - auth on title applies (within aggregate)", async () => {
        const query = /* GraphQL */ `
            {
                moviesConnection(first: 10) {
                    groupBy(fields: { released: true, other: true }) {
    ...
    test("Paginated aggregate on grouped movies, project nodes - auth on title applies (within aggregate and nodes projection)", async () => {
        const query = /* GraphQL */ `
            {
                moviesConnection(first: 10) {
                    groupBy(fields: { released: true, other: true }) {
    ...
    test("Aggregate operations on actors for grouped movies - auth on released, other, title, name apply", async () => {
        const query = /* GraphQL */ `
            {
                moviesConnection {
                    groupBy(fields: { released: true, other: true }) {
