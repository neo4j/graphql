    ...
    test("read allow protected interface relationship", async () => {
        ...
        expect(formatCypher(result.cypher)).toMatchInlineSnapshot(`
            ...
            CALL {
                ...
                WHERE apoc.util.validatePredicate(NOT ((exists((this_Comment:\`Comment\`)<-[:HAS_CONTENT]-(:\`User\`)) AND any(thisthis1 IN [(this_Comment:\`Comment\`)<-[:HAS_CONTENT]-(thisthis1:\`User\`) | thisthis1] WHERE (thisthis1.id IS NOT NULL AND thisthis1.id = $thisparam0)))), [0])
                ...
                WHERE apoc.util.validatePredicate(NOT ((exists((this_Post:\`Post\`)<-[:HAS_CONTENT]-(:\`User\`)) AND any(thisthis3 IN [(this_Post:\`Post\`)<-[:HAS_CONTENT]-(thisthis3:\`User\`) | thisthis3] WHERE (thisthis3.id IS NOT NULL AND thisthis3.id = $thisparam1)))), [0])
    ...
    test("Read Two Relationships", async () => {
        ...
        expect(formatCypher(result.cypher)).toMatchInlineSnapshot(`
            ...
            CALL {
                WITH this
                MATCH (this)-[thisthis0:HAS_CONTENT]->(this_Comment:\`Comment\`)
                WHERE (apoc.util.validatePredicate(NOT ((exists((this_Comment:\`Comment\`)<-[:HAS_CONTENT]-(:\`User\`)) AND any(thisthis1 IN [(this_Comment:\`Comment\`)<-[:HAS_CONTENT]-(thisthis1:\`User\`) | thisthis1] WHERE (thisthis1.id IS NOT NULL AND thisthis1.id = $thisparam0)))), [0]) AND this_Comment.id = $thisparam1)
                RETURN { __resolveType: \\"Comment\\" } AS content
                UNION
                WITH this
                MATCH (this)-[thisthis2:HAS_CONTENT]->(this_Post:\`Post\`)
                WHERE (apoc.util.validatePredicate(NOT ((exists((this_Post:\`Post\`)<-[:HAS_CONTENT]-(:\`User\`)) AND any(thisthis3 IN [(this_Post:\`Post\`)<-[:HAS_CONTENT]-(thisthis3:\`User\`) | thisthis3] WHERE (thisthis3.id IS NOT NULL AND thisthis3.id = $thisparam2)))), [0]) AND this_Post.id = $thisparam3)
                CALL {
                    WITH this_Post
                    MATCH (this_Post)-[thisthis4:HAS_COMMENT]->(this_Post_comments:\`Comment\`)
                    WHERE (this_Post_comments.id = $thisparam4 AND apoc.util.validatePredicate(NOT ((exists((this_Post_comments)<-[:HAS_CONTENT]-(:\`User\`)) AND any(thisthis5 IN [(this_Post_comments)<-[:HAS_CONTENT]-(thisthis5:\`User\`) | thisthis5] WHERE (thisthis5.id IS NOT NULL AND thisthis5.id = $thisparam5)))), \\"@neo4j/graphql/FORBIDDEN\\", [0]))
    ...
    test("Nested Update Node", async () => {
        ...
        expect(formatCypher(result.cypher)).toMatchInlineSnapshot(`
            ...
            CALL {
                ...
                WHERE apoc.util.validatePredicate(NOT ((exists((this_Comment:\`Comment\`)<-[:HAS_CONTENT]-(:\`User\`)) AND any(update_this1 IN [(this_Comment:\`Comment\`)<-[:HAS_CONTENT]-(update_this1:\`User\`) | update_this1] WHERE (update_this1.id IS NOT NULL AND update_this1.id = $update_param0)))), [0])
                RETURN { __resolveType: \\"Comment\\", id: this_Comment.id } AS content
                UNION
                WITH this
                MATCH (this)-[update_this2:HAS_CONTENT]->(this_Post:\`Post\`)
                WHERE apoc.util.validatePredicate(NOT ((exists((this_Post:\`Post\`)<-[:HAS_CONTENT]-(:\`User\`)) AND any(update_this3 IN [(this_Post:\`Post\`)<-[:HAS_CONTENT]-(update_this3:\`User\`) | update_this3] WHERE (update_this3.id IS NOT NULL AND update_this3.id = $update_param1)))), [0])
