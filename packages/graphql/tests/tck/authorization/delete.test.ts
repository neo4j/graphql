...
describe("Edge subquery", () => {
    ...
    beforeAll(() => {
        typeDefs = gql`
            type Movie
                @authorization(
                    filter: [{ where: { node: { title: "$jwt.sub" } } }]
                    validate: [{ where: { node: { title: "$jwt.sub" } } }]
                ) {
                title: String!
            ...
            type Actor
                @authorization(
                    filter: [{ where: { node: { name: "$jwt.sub" } } }]
                    validate: [{ where: { node: { name: "$jwt.sub" } } }]
                ) {
                name: String!
    ...
    test("Projecting node and relationship properties with no arguments", async () => {
        const query = gql`
            mutation {
                deleteMovies(delete: { actors: { where: {} } }) {
                    nodesDeleted
                }
            }
        `;
