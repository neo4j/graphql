...
describe("https://github.com/neo4j/graphql/issues/3929", () => {
    ...
    test("should not add an authorization check for connects coming from create", async () => {
        ...
        expect(formatCypher(result.cypher)).toMatchInlineSnapshot(`
            ...
            OPTIONAL MATCH (this_delete_members0)<-[:CREATOR_OF]-(authorization_this0:User)
            ...
            WHERE this_delete_members0.id = $updateGroups_args_delete_members0_where_this_delete_members0param0 AND apoc.util.validatePredicate(NOT ($isAuthenticated = true AND (creatorCount <> 0 AND ($jwt.uid IS NOT NULL AND authorization_this0.id = $jwt.uid))), \\"@neo4j/graphql/FORBIDDEN\\", [0])
