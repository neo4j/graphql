...
describe("https://github.com/neo4j/graphql/issues/4214", () => {
    ...
    test("should include checks for auth jwt param is not null", async () => {
        ...
        expect(formatCypher(result.cypher)).toMatchInlineSnapshot(`
            ...
            OPTIONAL MATCH (this0_transaction_connect0_node)-[:TRANSACTION]->(authorization_this0:Store)
            WITH *, count(authorization_this0) AS storeCount
            OPTIONAL MATCH (this0_transaction_connect0_node)-[:TRANSACTION]->(authorization_this1:Store)
            WITH *, count(authorization_this1) AS storeCount
            WITH *
            	WHERE this0_transaction_connect0_node.id = $this0_transaction_connect0_node_param0 AND ((($isAuthenticated = true AND ($jwt.roles IS NOT NULL AND $authorization_param2 IN $jwt.roles)) OR ($isAuthenticated = true AND ((($jwt.roles IS NOT NULL AND $authorization_param3 IN $jwt.roles) OR ($jwt.roles IS NOT NULL AND $authorization_param4 IN $jwt.roles)) AND (storeCount <> 0 AND ($jwt.store IS NOT NULL AND authorization_this0.id = $jwt.store))))) AND apoc.util.validatePredicate(NOT ($isAuthenticated = true AND ((($jwt.roles IS NOT NULL AND $authorization_param5 IN $jwt.roles) OR ($jwt.roles IS NOT NULL AND $authorization_param6 IN $jwt.roles)) AND (storeCount <> 0 AND ($jwt.store IS NOT NULL AND authorization_this1.id = $jwt.store)))), \\"@neo4j/graphql/FORBIDDEN\\", [0]))
            	CALL {
            		WITH *
            ...
            CALL {
                WITH this0
                MATCH (this0)-[:ITEM_TRANSACTED]->(authorization_this2:Transaction)
                OPTIONAL MATCH (authorization_this2)-[:TRANSACTION]->(authorization_this3:Store)
                WITH *, count(authorization_this3) AS storeCount
                WITH *
                WHERE (storeCount <> 0 AND ($jwt.store IS NOT NULL AND authorization_this3.id = $jwt.store))
                RETURN count(authorization_this2) = 1 AS authorization_var0
            }
            OPTIONAL MATCH (this0_transaction_connect0_node)-[:TRANSACTION]->(authorization_this1:Store)
            WITH *, count(authorization_this1) AS storeCount
            WITH *
            WHERE (apoc.util.validatePredicate(NOT ($isAuthenticated = true AND ((($jwt.roles IS NOT NULL AND $authorization_param2 IN $jwt.roles) OR ($jwt.roles IS NOT NULL AND $authorization_param3 IN $jwt.roles)) AND authorization_var0 = true)), \\"@neo4j/graphql/FORBIDDEN\\", [0]) AND apoc.util.validatePredicate(NOT ($isAuthenticated = true AND ((($jwt.roles IS NOT NULL AND $authorization_param4 IN $jwt.roles) OR ($jwt.roles IS NOT NULL AND $authorization_param5 IN $jwt.roles)) AND (storeCount <> 0 AND ($jwt.store IS NOT NULL AND authorization_this1.id = $jwt.store)))), \\"@neo4j/graphql/FORBIDDEN\\", [0]))
            ...
            CALL {
                WITH this0
                MATCH (this0)-[:ITEM_TRANSACTED]->(authorization_0_0_this1:Transaction)
                OPTIONAL MATCH (authorization_0_0_this1)-[:TRANSACTION]->(authorization_0_0_this2:Store)
                WITH *, count(authorization_0_0_this2) AS storeCount
                WITH *
                WHERE (storeCount <> 0 AND ($jwt.store IS NOT NULL AND authorization_0_0_this2.id = $jwt.store))
                RETURN count(authorization_0_0_this1) = 1 AS authorization_0_0_var0
            }
            WITH *
            WHERE apoc.util.validatePredicate(NOT ($isAuthenticated = true AND ((($jwt.roles IS NOT NULL AND $authorization_0_0_param2 IN $jwt.roles) OR ($jwt.roles IS NOT NULL AND $authorization_0_0_param3 IN $jwt.roles)) AND authorization_0_0_var0 = true)), \\"@neo4j/graphql/FORBIDDEN\\", [0])
            RETURN this0
            }
            CALL {
                WITH this0
        ...
        expect(formatParams(result.params)).toMatchInlineSnapshot(`
            "{
                ...
                \\"authorization_param4\\": \\"store-owner\\",
                \\"authorization_param5\\": \\"employee\\",
                \\"authorization_param6\\": \\"employee\\",
