...
describe("https://github.com/neo4j/graphql/issues/4170", () => {
    ...
    test("should include checks for auth jwt param is not null", async () => {
        ...
        expect(formatCypher(result.cypher)).toMatchInlineSnapshot(`
            ...
            CALL {
                WITH this0_settings0_node_openingDays0_node_open0_node
                MATCH (this0_settings0_node_openingDays0_node_open0_node)<-[:HAS_OPEN_INTERVALS]-(authorization_0_3_this1:OpeningDay)
                CALL {
                    WITH authorization_0_3_this1
                    MATCH (authorization_0_3_this1)<-[:VALID_GARAGES]-(authorization_0_3_this2:Settings)
            ...
            CALL {
                WITH this0_settings0_node_openingDays0_node
                MATCH (this0_settings0_node_openingDays0_node)<-[:VALID_GARAGES]-(authorization_0_2_this1:Settings)
                OPTIONAL MATCH (authorization_0_2_this1)<-[:HAS_SETTINGS]-(authorization_0_2_this2:Tenant)
                WITH *, count(authorization_0_2_this2) AS tenantCount
                WITH *
                WHERE (tenantCount <> 0 AND size([(authorization_0_2_this2)<-[:ADMIN_IN]-(authorization_0_2_this3:User) WHERE ($jwt.id IS NOT NULL AND authorization_0_2_this3.userId = $jwt.id) | 1]) > 0)
                RETURN count(authorization_0_2_this1) = 1 AS authorization_0_2_var0
            }
            OPTIONAL MATCH (this0_settings0_node)<-[:HAS_SETTINGS]-(authorization_0_1_this1:Tenant)
            WITH *, count(authorization_0_1_this1) AS tenantCount
            WITH *
            WHERE apoc.util.validatePredicate(NOT ($isAuthenticated = true AND authorization_0_3_var0 = true), \\"@neo4j/graphql/FORBIDDEN\\", [0]) AND apoc.util.validatePredicate(NOT ($isAuthenticated = true AND authorization_0_2_var0 = true), \\"@neo4j/graphql/FORBIDDEN\\", [0]) AND apoc.util.validatePredicate(NOT ($isAuthenticated = true AND (tenantCount <> 0 AND size([(authorization_0_1_this1)<-[:ADMIN_IN]-(authorization_0_1_this0:User) WHERE ($jwt.id IS NOT NULL AND authorization_0_1_this0.userId = $jwt.id) | 1]) > 0)), \\"@neo4j/graphql/FORBIDDEN\\", [0]) AND apoc.util.validatePredicate(NOT ($isAuthenticated = true AND size([(this0)<-[:ADMIN_IN]-(authorization_0_0_this0:User) WHERE ($jwt.id IS NOT NULL AND authorization_0_0_this0.userId = $jwt.id) | 1]) > 0), \\"@neo4j/graphql/FORBIDDEN\\", [0])
            RETURN this0
            }
            CALL {
                WITH this0
