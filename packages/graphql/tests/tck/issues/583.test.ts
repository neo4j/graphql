        ...
        expect(formatCypher(result.cypher)).toMatchInlineSnapshot(`
            ...
            CALL {
                ...
                CALL {
                    ...
                    WITH this_actedIn { __resolveType: \\"Movie\\", __id: id(this), .title, .awardsGiven } AS this_actedIn
                    RETURN this_actedIn AS this_actedIn
                    UNION
                    WITH *
                    MATCH (this)-[this1:ACTED_IN]->(this_actedIn:\`Series\`)
                    WITH this_actedIn { __resolveType: \\"Series\\", __id: id(this), .title, .awardsGiven } AS this_actedIn
                    RETURN this_actedIn AS this_actedIn
                    UNION
                    WITH *
                    MATCH (this)-[this2:ACTED_IN]->(this_actedIn:\`ShortFilm\`)
                    WITH this_actedIn { __resolveType: \\"ShortFilm\\", __id: id(this), .title } AS this_actedIn
                    RETURN this_actedIn AS this_actedIn
                }
                WITH this_actedIn
