...
describe("https://github.com/neo4j/graphql/issues/2022", () => {
    ...
    test("query nested relations under a root connection field", async () => {
        ...
        //     }
        //     CALL apoc.util.validate(NOT (apoc.util.validatePredicate(NOT ($auth.isAuthenticated = true), \\"@neo4j/graphql/UNAUTHENTICATED\\", [0])), \\"@neo4j/graphql/FORBIDDEN\\", [0])
        //     RETURN this { .id, .name, bussing: [this_bussing IN apoc.cypher.runFirstColumnMany(\\"MATCH (this)-[:HAS_HISTORY]->(:ServiceLog)-[:HAS_BUSSING]->(records:BussingRecord)-[:BUSSED_ON]->(date:TimeGraph)
        //     WITH DISTINCT records, date LIMIT $limit
        //     RETURN records ORDER BY date.date DESC\\", {this: this, auth: $auth, limit: $this_bussing_limit}) | this_bussing { .id, .attendance, markedAttendance:  apoc.cypher.runFirstColumnSingle(\\"MATCH (this)<-[:PRESENT_AT_SERVICE|ABSENT_FROM_SERVICE]-(member:Member)
        //     RETURN COUNT(member) > 0 AS markedAttendance\\", {this: this_bussing, auth: $auth}), serviceDate: this_bussing_serviceDate }] } as this"
        // `);
