    ...
    test("should include checks for auth jwt param is not null", async () => {
        ...
        expect(formatCypher(result.cypher)).toMatchInlineSnapshot(`
            ...
            CALL {
                ...
                MATCH (this0_openingDays_connect0_node)<-[:VALID_OPENING_DAYS]-(authorization_this1:Settings)
                OPTIONAL MATCH (authorization_this1)<-[:HAS_SETTINGS]-(authorization_this2:Tenant)
                WITH *, count(authorization_this2) AS tenantCount
                WITH *
                WHERE (tenantCount <> 0 AND size([(authorization_this2)<-[:ADMIN_IN]-(authorization_this3:User) WHERE ($jwt.id IS NOT NULL AND authorization_this3.userId = $jwt.id) | 1]) > 0)
                RETURN count(authorization_this1) = 1 AS authorization_var0
            }
            WITH *
            	WHERE this0_openingDays_connect0_node.id = $this0_openingDays_connect0_node_param0 AND apoc.util.validatePredicate(NOT ($isAuthenticated = true AND authorization_var0 = true), \\"@neo4j/graphql/FORBIDDEN\\", [0])
            	CALL {
            		WITH *
            ...
            CALL {
                WITH this0_openingDays_connect0_node
                MATCH (this0_openingDays_connect0_node)<-[:VALID_OPENING_DAYS]-(authorization_this3:Settings)
                OPTIONAL MATCH (authorization_this3)<-[:HAS_SETTINGS]-(authorization_this4:Tenant)
                WITH *, count(authorization_this4) AS tenantCount
                WITH *
                WHERE (tenantCount <> 0 AND size([(authorization_this4)<-[:ADMIN_IN]-(authorization_this5:User) WHERE ($jwt.id IS NOT NULL AND authorization_this5.userId = $jwt.id) | 1]) > 0)
                RETURN count(authorization_this3) = 1 AS authorization_var2
            }
            WITH *
            WHERE (apoc.util.validatePredicate(NOT ($isAuthenticated = true AND (hostCount <> 0 AND size([(authorization_this1)<-[:ADMIN_IN]-(authorization_this0:User) WHERE ($jwt.id IS NOT NULL AND authorization_this0.userId = $jwt.id) | 1]) > 0)), \\"@neo4j/graphql/FORBIDDEN\\", [0]) AND apoc.util.validatePredicate(NOT ($isAuthenticated = true AND authorization_var2 = true), \\"@neo4j/graphql/FORBIDDEN\\", [0]))
