...
export function describeIf(condition: boolean, message?: string): jest.Describe {
    if (!condition && message) {
        console.log(message);
    }
...
describeIf.only = function (condition: boolean, message?: string): jest.Describe {
    if (!condition && message) {
        console.log(message);
    }

    return condition ? describe.only : describe.only.skip;
};

// eslint-disable-next-line @typescript-eslint/no-unused-vars
describeIf.skip = function (_condition: boolean, _message?: string): jest.Describe {
