...
describe("Subscription authentication with @authentication(SUBSCRIBE)", () => {
    ...
    test("a validly signed admin token receives the event", async () => {
        ...
        await wsClient.subscribe(`
            subscription {
                ${typeMovie.operations.subscribe.created}(where: { title: "admin-token-movie" }) {
                    ...
                    filter: [
                        {
                            ...
                            where: { node: { title: "$context.connectionParams.allowedTitle" } }
                        }
                    ]
                ) {
                title: String!
...
describe("Subscription authentication with a developer-injected top-level context.jwt", () => {
    ...
    test("a developer-injected top-level context.jwt receives the event with no client credential", async () => {
        ...
        await wsClient.subscribe(`
            subscription {
                ${typeMovie.operations.subscribe.created}(where: { title: "developer-jwt-movie" }) {
                    ${typeMovie.operations.subscribe.payload.created} {
                        title
                    }
                }
        ...
        await createMovie("developer-jwt-movie");
        await waitForMaybeEvent(wsClient);
        ...
        expect(wsClient.events).toEqual([
            {
                [typeMovie.operations.subscribe.created]: {
                    [typeMovie.operations.subscribe.payload.created]: { title: "developer-jwt-movie" },
                },
            },
        ]);
    });
