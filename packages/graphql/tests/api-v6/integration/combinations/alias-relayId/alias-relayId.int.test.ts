...
describe("RelayId projection with alias directive", () => {
    ...
    beforeAll(async () => {
        const typeDefs = /* GraphQL */ `
            type ${Movie} @node {
                dbId: ID! @id @unique(constraintName: "MOVIE_ID_UNIQUE") @relayId @alias(property: "serverId")
                title: String!
            ...
            type ${Actor} @node {
                dbId: ID! @id @unique(constraintName: "ACTOR_ID_UNIQUE") @relayId @alias(property: "serverId")
                name: String!
            }
        `;
