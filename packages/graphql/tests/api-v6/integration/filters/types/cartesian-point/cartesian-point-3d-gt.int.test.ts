...
describe.skip("CartesianPoint 3d GT", () => {
    ...
    test("wgs-84-3d point filter by GT", async () => {
        // distance is in meters
        const distance = 1000 * 1000; // 1000 km
        const query = /* GraphQL */ `
            query  {
                ${Location.plural}(where: { edges: { node: { value: { gt: { point: { x: ${Paris.x}, y: ${Paris.y}, z: ${Paris.z} }, distance: ${distance} } } } } }) {
                    connection {
                        edges {
                            node {
                                id
    ...
    test("wgs-84-3d point filter by NOT GT", async () => {
        // distance is in meters
        const distance = 1000 * 1000; // 1000 km
        const query = /* GraphQL */ `
            query  {
                ${Location.plural}(where: { edges: { node: { value: { NOT: { gt: { point: { x: ${Paris.x}, y: ${Paris.y}, z: ${Paris.z} }, distance: ${distance} } } } } } }) {
                    connection {
                        edges {
                            node {
                                id
