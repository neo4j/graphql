...
describe.skip("CartesianPoint 2d IN", () => {
    ...
    test("wgs-84-2d point filter by IN", async () => {
        const query = /* GraphQL */ `
            query {
                ${Location.plural}(where: { edges: { node: { value: { in: [{ x: ${Rome.x}, y: ${Rome.y} }, { x: ${Paris.x}, y: ${Paris.y} }]  } } } }) {
                    connection {
                        edges {
                            node {
                                id
    ...
    test("wgs-84-2d point filter by NOT IN", async () => {
        const query = /* GraphQL */ `
            query {
                ${Location.plural}(where: { edges: { node: { value: { NOT: { in: [{ x: ${Rome.x}, y: ${Rome.y} }, { x: ${Paris.x}, y: ${Paris.y} }]  } } } } }) {
                    connection {
                        edges {
                            node {
                                id
