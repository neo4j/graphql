...
describe.skip("Point 2d LT", () => {
    ...
    test("wgs-84-2d point filter by LT", async () => {
        const query = /* GraphQL */ `
            query types($longitude: Float!, $latitude: Float!, $distance: Float!) {
                ${Location.plural}(where: { edges: { node: { value: { lt: { point: { longitude: $longitude, latitude: $latitude }, distance: $distance } } } } }) {
                    connection {
                        edges {
                            node {
                                id
    ...
    test("wgs-84-2d point filter by NOT LT", async () => {
        const query = /* GraphQL */ `
            query types($longitude: Float!, $latitude: Float!, $distance: Float!) {
                ${Location.plural}(where: { edges: { node: { value: { NOT:  { lt: { point: { longitude: $longitude, latitude: $latitude }, distance: $distance } } } } } }) {
                    connection {
                        edges {
                            node {
                                id
