...
describe("aggregations-top_level authorization", () => {
    ...
    test("should throw when invalid allow when aggregating a Int field", async () => {
        ...
        const typeDefs = /* GraphQL */ `
            type ${Movie} @node {
                ...
                imdbRatingInt: Int
                    @authorization(validate: [{ when: BEFORE, where: { node: { director_SINGLE: { id_EQ: "$jwt.sub" } } } }])
            }
        ...
        const query = `
            {
                ${Movie.operations.aggregate}(where: {id_EQ: "${movieId}"}) {
                    imdbRatingInt {
                        min
                        max
                    }
    ...
    test("should throw when invalid allow when aggregating a ID field", async () => {
        ...
        await testHelper.executeCypher(`
                CREATE (:${Person} {id: "${userId}"})-[:DIRECTED]->(:${Movie} {id: "${movieId}", someStr: "some-random-string"})
            `);
