...
describe("cypher directive filtering - relationship auth filter", () => {
    ...
    // TODO: FIXME
    // Skipped as the order between validatePredicate and other filters is not guaranteed, causing flaky tests.
    // Fixed on 7.x by the migration to apoc.util.validate.
    test.skip("authorization should be executed only for movies with ratings lower than 7", async () => {
        const Movie = testHelper.createUniqueType("Movie");
        const Actor = testHelper.createUniqueType("Actor");
