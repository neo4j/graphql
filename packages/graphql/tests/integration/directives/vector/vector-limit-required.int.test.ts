...
describe("@vector directive - limit required", () => {
    ...
    test("error limit not provided", async () => {
        // Skip if vector not supported
        if (!VECTOR_SUPPORT) {
            console.log("VECTOR SUPPORT NOT AVAILABLE - SKIPPING");
            return;
        ...
        const query = `
                query($vector: [Float!]) {
                    ${queryName}(vector: $vector, after: "${cursor}") {
                        edges {
                            score
                            node {
                                title
        ...
        expect(gqlResult.errors).toIncludeSameMembers([
            new GraphQLError(`Field "${queryName}" argument "first" of type "Int!" is required, but it was not provided.`),
         ]);
    });
});
