...
describe("@vector directive", () => {
    ...
    describe("Query tests with auth", () => {
        ...
        test("Multiple vector index fields", async () => {
            // Skip if multi-db not supported
            if (!MULTIDB_SUPPORT) {
                console.log("MULTIDB_SUPPORT NOT AVAILABLE - SKIPPING");
                return;
            ...
            const typeDefs = `
                    ...
                    type ${movieType.name} @vector(indexes: [{ indexName: "${movieType.name}Index", fields: ["title", "description"] }]) {
                        title: String!
                        description: String
                        released: Int!
                        actors: [${personType.name}!]! @relationship(type: "ACTED_IN", direction: IN)
    ...
    describe("Index Creation", () => {
        ...
        test("Creates index if it doesn't exist", async () => {
            ...
            const typeDefs = gql`
            type ${type.name} @vector(indexes: [{ indexName: "${indexName1}", fields: ["title"] }]) {
                    title: String!
                }
            `;
        ...
        test("Creates two index's if they dont exist", async () => {
            ...
            const typeDefs = gql`
                type ${type.name} @vector(indexes: [{ indexName: "${indexName1}", fields: ["title"] }, { indexName: "${indexName2}", fields: ["description"] }]) {
                    title: String!
                    description: String!
                }
            `;
        ...
        test("When using the node label, creates index if it doesn't exist", async () => {
            ...
            const typeDefs = gql`
                type ${type.name} @vector(indexes: [{ indexName: "${indexName1}", fields: ["title"] }]) @node(labels: ["${label}"]) {
                    title: String!
                }
            `;
        ...
        test("Throws when missing index (create index and constraint option not true)", async () => {
            ...
            ).rejects.toThrow(`Missing @vector index '${indexName1}' on Node '${type.name}'`);
        });
        ...
        test("Throws when an index is missing fields", async () => {
            ...
            const typeDefs = gql`
                type ${type.name} @vector(indexes: [{ indexName: "${indexName1}", fields: ["title", "description"] }]) {
                    title: String!
                    description: String!
                }
            `;
            ...
            await testHelper.executeCypher(
                [`CREATE VECTOR INDEX ${indexName1}`, `IF NOT EXISTS FOR (n:${type.name})`, `ON EACH [n.title]`].join(
                    " "
                )
            );
            ...
            ).rejects.toThrow(`@vector index '${indexName1}' on Node '${type.name}' is missing field 'description'`);
        });
        ...
        test("When using the field alias, throws when index is missing fields", async () => {
            ...
            await testHelper.executeCypher(
                [
                    `CREATE VECTOR INDEX ${indexName1}`,
            ...
            await expect(
                ...
                `@vector index '${indexName1}' on Node '${type.name}' is missing field 'description' aliased to field '${aliasName}'`
            );
        });
        ...
        test("Throws when index is missing fields when used with create option", async () => {
            ...
            await expect(
                ...
                `@vector index '${indexName1}' on Node '${type.name}' already exists, but is missing field 'description'`
            );
        });
        ...
        test("Create index for ID field if it doesn't exist", async () => {
            ...
            const typeDefs = gql`
                type ${type.name} @vector(indexes: [{ indexName: "${indexName1}", fields: ["id"] }]) {
                    id: ID!
                }
            `;
        ...
        test("should not throw if index exists on an additional label", async () => {
            ...
            const typeDefs = `
                type ${baseType.name} @node(labels: ["${baseType.name}", "${additionalType.name}"]) @vector(indexes: [{ indexName: "${indexName1}", fields: ["title"] }]) {
                    title: String!
                }
            `;
