...
describe("aggregations nested field", () => {
    ...
    // This test is used to highlight the difference between the two syntax
    // aggregate inside connections should use distinct while the legacy syntax not, see: https://github.com/neo4j/graphql/issues/6005
    test.only("should return Post and project only users who liked exactly 2 posts (legacy vs connection style)", async () => {
        const post1Title = "Post 1";
        const post2Title = "Post 2";
        const post3Title = "Post 3";
        const name1 = "User 1";
        ...
        expect((gqlResult.data as any)[Post.plural]).toIncludeSameMembers([
            {
                title: post1Title,
                likesNewStyle: expect.toIncludeSameMembers([{ name: name1 }, { name: name2 }]),
                likesLegacyStyle: expect.toIncludeSameMembers([ { name: name2 }]),
            },
            {
                title: post2Title,
                likesNewStyle: expect.toIncludeSameMembers([{ name: name1 }, { name: name2 }]),
