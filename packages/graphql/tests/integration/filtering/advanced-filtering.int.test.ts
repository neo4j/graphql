    ...
    describe("Relationship/Connection Filtering", () => {
        describe("equality", () => {
            test("should find using relationship equality on node", async () => {
                ...
                const query = /* GraphQL */ `
                    {
                        ${randomType1.plural}(where: { ${randomType2.plural}: { some: {id: { equals: "${relationId}" }} } }) {
                            id
                            ${randomType2.plural} {
                                id
                            }
            ...
            test("should find using equality on node using connection", async () => {
                ...
                const query = /* GraphQL */ `
                    {
                        ${Movie.plural}(where: { genresConnection: { some: {  node: { id: { equals: "${genreId}" } } } } }) {
                            id
                            genres {
                                id
                            }
            ...
            test("should find using equality on relationship using connection", async () => {
                ...
                const query = /* GraphQL */ `
                    {
                        ${Movie.plural}(where: { genresConnection: { some: { edge: { id: { equals: "${actedInId}" } } } } }) {
                            id
                            genres {
                                id
                            }
            ...
            test("should find relationship and node property equality using connection", async () => {
                ...
                const query = /* GraphQL */ `
                        {
                            ${Movie.plural}(where: { genresConnection: { some: { node: { id: { equals: "${genreId}" }} edge: { id: { equals: "${actedInId}" } } } } }) {
                                id
                                genres {
                                    id
                                }
        ...
        describe("NOT", () => {
            test("should find using NOT on relationship", async () => {
                ...
                const query = /* GraphQL */ `
                        {
                            ${randomType1.plural}(where: { NOT: { ${randomType2.plural}: { some: { id: { equals: "${relationId2}" }}} } }) {
                                id
                                ${randomType2.plural} {
                                    id
                                }
            ...
            test("should find using relationship properties and connections", async () => {
                ...
                const query = /* GraphQL */ `
                        {
                            ${randomType1.plural}(where: { ${randomType2.plural}Connection: { none: {edge: { id: { equals: "${actedInId}"} } } } }) {
                                id
                                ${randomType2.plural} {
                                    id
                                }
        ...
        describe("List Predicates", () => {
            ...
            describe("on connection", () => {
                const generateQuery = (predicate: "all" | "none" | "single" | "some") => /* GraphQL */ `
                    query($movieIds: [ID!]!) {
                        ${Movie.plural}(where: { AND: [{ id: {in: $movieIds} }, { actorsConnection: { ${predicate}: { node: { NOT: { flag: { equals: false} } } }}}] }) {
                            id
                            actors(where: { NOT: { flag:{ equals: false }} }) {
                                id
                                flag
