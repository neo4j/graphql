...
describe("typename_IN", () => {
    ...
    test("connection", async () => {
        const query = `
        {
            productionsConnection(where: { OR: [ { typename_IN: [${Movie.name}, ${Series.name}] } { typename_IN: [${Cartoon.name}] } ] }) {
                edges {
                    node {
                        title
                    }
    ...
    test("nested connection", async () => {
        ...
        {
            ${Actor.plural} {
                actedInConnection(where: { NOT:  { typename_IN: [${Movie.name}, ${Series.name}] } }) {
                    edges {
                        node {
                            title
                        }
