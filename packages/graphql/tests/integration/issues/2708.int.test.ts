...
describe("https://github.com/neo4j/graphql/issues/2708", () => {
    ...
    test("should find genres with multiple AND aggregates", async () => {
        const query = /* GraphQL */ `
            {
                ${movieType.plural}(where: { genres_SOME: { AND: [{ moviesAggregate: { count: 2 } }, { seriesAggregate: { node: { name_SHORTEST_EQUAL: ${seriesName1.length} } } }] } }) {
                    title
                }
            }
        `;
    ...
    test("should find genres with multiple OR aggregates", async () => {
        const query = /* GraphQL */ `
            {
                ${movieType.plural}(where: { genres_SOME: { OR: [{ moviesAggregate: { count: 3 } }, { seriesAggregate: { node: { name_SHORTEST_EQUAL: ${seriesName1.length} } } }] } }) {
                    title
                }
            }
        `;
    ...
    test("should find genres with multiple implicit AND aggregates", async () => {
        const query = /* GraphQL */ `
            {
                ${movieType.plural}(where: { genres_SOME: { moviesAggregate: { count: 2 }, seriesAggregate: { node: { name_SHORTEST_EQUAL: ${seriesName1.length} } } } }) {
                    title
                }
            }
        `;
    ...
    test("should find genres with aggregation at the same level", async () => {
        const query = /* GraphQL */ `
            {
                ${movieType.plural}(where: { genres_SOME: { moviesAggregate: { count: 3 } }, genresAggregate: { count: 1 } }) {
                    title
                }
            }
        `;
