...
describe("https://github.com/neo4j/graphql/issues/6805", () => {
    ...
    test("auth rules are applied and pass after the nested connection is made", async () => {
    ...
    test("auth rules are applied and fail after the nested connection is made", async () => {
        ...
        expect((queryResult.errors as any[])[0].message).toBe("Forbidden");

        await testHelper.expectRelationship(Invitee, Group, "INVITED_TO").toNotExist();
    });
});
