...
CALL () {
    CALL {
        MATCH (this5:Movie)
        RETURN this5 AS node
        UNION
        MATCH (this6:MovieClone)
        RETURN this6 AS node
    }
    WITH DISTINCT node
    RETURN { min: min(node.released), max: max(node.released) } AS this7
}
WITH edges, { node: { title: this4, released: this7 } } AS var8
WITH edges, size(edges) AS totalCount, var8
RETURN { edges: edges, totalCount: totalCount, aggregate: var8 } AS this