...
describe("OGM", () => {
    ...
    describe("generics", () => {
        test("should allow the user to use there own generic with the model class without typescript errors", async () => {
            const session = driver.session();
            ...
            class MyCustomModel {
                ...
                }): Promise<{ users: [{ id: string; password: string }] }> {
                    return { users: [{ id: "some-id", password: "some-password" }] };
                }
            }
            ...
            try {
                const { users } = await User.create({ input: [{ id, password }] });
