...
the variable `$jwt` to define a custom label define in the jwt token
(similarly to how it is used in the xref::auth/index.adoc[@auth directive]):

[source, graphql, indent=0]
----
type User @node(label: "$jwt.sud") {
    name: String!
}
----

The following query will yield a different cypher query depending on the user JWT token
...
Assuming an user with `"sud": 1234`, the cypher query will look like:
...
new ApolloServer({
    ...
    context: ({ req }: any) => ({ req, appId: "1234" }),
});
----
