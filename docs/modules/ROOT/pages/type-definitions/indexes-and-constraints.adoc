...
For every type with the `@fulltext` directive, the `fulltext` argument will be available for `Connect` and `Aggregate` queries.

The fulltext argument accepts an object with 2 parameters:

* `index`: An enum value to select the fulltext index to use. If only 1 index is defined por that type it will default to it.
* `phrase`: The text to match with the given index.


This argument can then be used to perform a https://lucene.apache.org/[Lucene full-text query] to match and return products. Below is an example of this:

[source, graphql, indent=0]
----
query {
  productsConnection(fulltext: { phrase: "Hot sauce", index: ProductName }) {
    edges {
      node {
        name
      }
...
This query will only return products matching the given phrase and index.
