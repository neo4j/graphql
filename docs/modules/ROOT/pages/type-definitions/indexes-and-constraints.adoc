...
Once you specify the index, you will now gain a 'Top Level' `search` key on the following operations: 
...
Here we use the `search` key, and the phrase is using https://lucene.apache.org/[Lucene’s full-text query language] to match and return Products: 

[source, graphql, indent=0]
----
query {
    products(search: { ProductName: { phrase: "beer OR cerveza" } }) {
        name
    }
}
----
...
----
query {
    products(search: { ProductName: { phrase: "beer OR cerveza", score_EQUAL: 10 } }) {
        name
    }
}
----

> Note that you can only query one Fulltext index at once and that the search key is only available on 'Top Level' queries. 
