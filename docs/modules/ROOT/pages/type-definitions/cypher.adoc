...
The new `experimental` flag will change the translation of custom Cypher. Instead of using https://neo4j.com/labs/apoc/4.0/overview/apoc.cypher/apoc.cypher.runFirstColumnMany/[apoc.cypher.runFirstColumnMany] it will directly wrap the query within a `CALL { }` subquery. This behvaiour has proven to be much more performant for the same queries, however, it may lead to unexpected changes, mainly if using Neo4j 5.x, where the subqueries need to be _aliased_.
...
----
type query {
    test: String! @cypher(statement: "RETURN 'hello'", experimental: true)
}
----
...
----
type query {
    test: String! @cypher(statement: "RETURN 'hello' as result", experimental: true)
}
----


Additionally, escaping strings is no longer needed with the `experimental` flag.
