...
For instance, let's say that we want to avoid all the top level aggregation operations to be generated. To avoid useless repetition, Neo4j GraphQL Library allows to use schema extension with `@query`, `@mutation`, `@subscription`, as follows:
