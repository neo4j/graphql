...
This definition can be used along with relationship fields. 
When an operation over a field is disabled this way, that relationship will not be available on top-level operations. 
For example:
...
There are several operations available for every relationship created using the directive `@relationship` as `create`, `connect`,  `disconnect`, `connectOrCreate`, `delete`, and `aggregate`. Not always these operations are needed, in fact, `@relationship` allows us to define which operations should be available for a relationship using the arguments `nestedOperations`, and `aggregate`.
...
NOTE: Please note that aggregate default value will be true in the 4.0.0 version of the library. See xref::guides/v4-migration/index.adoc#_relationship_changes[`@relationship changes`]

=== Usage


Disable aggregation and enable only create as nested operations:
...
With this directive, fields will only be featured in mutations for creating in and object types for querying.
