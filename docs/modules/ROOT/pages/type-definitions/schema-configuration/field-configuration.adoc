...
There are several operations available for every relationship created using the directive `@relationship` as `create`, `connect`, `disconnect`, `connectOrCreate`, `delete`, and `aggregate`. 
However, these operations are not always needed as the `@relationship` directive allows you to define which operations should be available for a relationship by using the arguments `nestedOperations` and `aggregate`.
