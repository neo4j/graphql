...
Note: The `@callback` directive has been deprecated. Please use the xref::directives.adoc#populated-by-directive[`@populatedBy` directive] instead.
...
The `@populatedBy` directive is used to specify a callback function that will be invoked and used to populate the given field when updating or creating the properties on a node or relationship.
The callback function will be executed after a GraphQL query has been parsed, before data is sent to the database.
