...
The class `Neo4jGraphQLSubscriptionsSingleInstancePlugin` is no longer exported, instead the default subscriptions behaviour can be enabled by setting the `subscriptions` option to `true` 
...
The underlying subscription system has not changed, and custom behaviour can be implemented the same way, by creating a class implementing the interface described in xref::subscriptions/mechanisms.adoc#custom-subscription[Subscription Mechanisms]
