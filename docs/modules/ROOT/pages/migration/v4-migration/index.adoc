...
If you were previously using the `@neo4j/graphql-plugins-subscriptions-amqp` package, this has been changed to `@neo4j/graphql-subscriptions-engine-amqp` to reflect this underlying change.
...
[source, javascript, indent=0]
----
import { Neo4jGraphQLSubscriptionsAMQPEngine } from "@neo4j/graphql-subscriptions-engine-amqp";
----
