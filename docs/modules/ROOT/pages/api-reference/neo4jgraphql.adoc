...
Asynchronous method which generates the GraphQL schema to be used in a server. Thr result is memoized, so if this is called twice, the schema is only generated once.
