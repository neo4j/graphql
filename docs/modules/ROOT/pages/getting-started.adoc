...
== (Advanced) Setup Websockets & Subscriptions with Apollo

Setting up subscriptions is only necessary if you want to receive real-time, authenticated, and authorized updates on your client.

. Install the websocket transport for subscriptions:
+
[source, bash, indent=0]
----
npm subscriptions-transport-ws --save
----
+
. Below `new Neo4jGraphql(...)` setup the subscription server:
+
[source, javascript, indent=0]
----
...
...
const subscriptionServer = SubscriptionServer.create({
    ...
    execute,
    subscribe,
    async onConnect(connectionParams, webSocket, context, info) {
      // Websocket connected here
      return neoSchema.onSubscriptionConnect(context);
    },
}, {
    // Express (from createServer(app)):
    server: httpServer
    // Fastify:
...
----
+
. Visiting `localhost:4000/api/graphql` will now allow you to listen to subscriptions. See the xref::subscriptions.adoc[Subscriptions guide.]
+
. For additional information please visit the https://www.apollographql.com/docs/apollo-server/data/subscriptions/[Apollo Guide]