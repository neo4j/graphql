...
This is supported by the link:https://www.npmjs.com/package/@neo4j/graphql-subscriptions-engine-amqp[@neo4j/graphql-subscriptions-engine-amqp] package.

The `@neo4j/graphql-subscriptions-engine-amqp` plugin connects to message brokers through the `AMQP 0-9-1` protocol to distribute subscription events across all server instances. 
...
const amqpSubscription = new Neo4jGraphQLSubscriptionsAMQPEngine({
    connection: {
        hostname: "localhost",
        username: "guest",
        password: "guest",
