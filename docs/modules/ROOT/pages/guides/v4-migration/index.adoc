...
In version 4.0.0, to comply with future API design and flexibility, fulltext arguments have been changed, requiring now a separate `index` and `phrase` fields, instead of the nested object as before.

Also, fulltext is only available in `connection` and `aggregation` queries, deprecating the simple API usage. 
...
----
query {
  movieConnection(fulltext: { index: movieTitleIndex, phrase: "Some Title" }) {
    edges {
      node {
        title
      }
...
NOTE: Top level Fulltext queries have also been deprecated due to limitations on its design, limiting the scope in which these could be used
