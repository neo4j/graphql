...
* xref::type-definitions/access-control.adoc[`@exclude`]: Skip generation of specified Query/Mutation fields for an object type
* xref::type-definitions/access-control.adoc[`@ignore`]: Ignore a field, which will need custom logic for resolution
...
There is an equivalent directive in the new library, but it does not work using database constraints as per the old library. See xref::type-definitions/autogeneration.adoc[`@id`].
...
Supported as you would expect, with additional xref::type-definitions/types.adoc[`BigInt`] support for 64 bit integers.
...
See xref::type-definitions/types.adoc[Temporal Types].

=== Spatial Types

The single type in `neo4j-graphql-js`, `Point`, has been split out into two types:

* xref::type-definitions/types.adoc[`Point`]
* xref::type-definitions/types.adoc[`CartesianPoint`]
...
Supported, queryable using inline fragments as per `neo4j-graphql-js`, but can also be created using Nested Mutations. See xref::type-definitions/unions-and-interfaces.adoc[Union Types].
