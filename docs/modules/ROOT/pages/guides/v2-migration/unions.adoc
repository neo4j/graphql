...
Furthermore, the where argument used now dictates which union members are returned from the database, to prevent overfetching. Please see xref::type-definitions/unions-and-interfaces.adoc[Querying a union] for background and explanation of this decision.
