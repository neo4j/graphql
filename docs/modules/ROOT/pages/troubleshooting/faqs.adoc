...
Currently, Neo4j GraphQL enforces on creating and updating that a movie has a director: 
...
However, at this point, there is no mechanism to support validating the actors relationship. Furthermore, there is a known limitation and that is: 

Given we create a movie and a director in one mutation: 
...
Then we delete the director node: 
...
No error is thrown, even though the schema states that all movies must have a director thus technically rendering the movie node we created invalid. 
