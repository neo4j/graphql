...
The following comparison operators are available for numeric types (`Int`, `Float`, <<type-definitions-types-bigint>>), <<type-definitions-types-temporal>> and <<type-definitions-types-spatial>>:
...
Filtering of spatial types is different to filtering of numerical types and also offers an additional filter - see <<type-definitions-types-spatial>>.
